    ...
    <div class="px-20 py-10">
        <v-form ref="form" class="w-full md:grid grid-cols-12">
            ...
            <div class="col-span-6">
                <div>
                    <div class="pb-10">
                        <h3 class="font-semibold text-lg text-[#2C4FBE]">ព័ត៌មានប័ណ្ណ</h3>
                        <div class="w-full h-[1px] bg-[#9D9D9D]" />
                    </div>
                    <v-select v-model="cardType" label="ប្រភេទប័ណ្ណ" :items="cardTypeData" variant="outlined"
                        :rules="fieldRule"></v-select>
                    <v-text-field v-model="cardId" variant="outlined" :rules="fieldRule" label="លេខប័ណ្ណ"
                        required></v-text-field>
                    <v-select v-model="weaponType" label="ប្រភេទអាវុធ" :items="weaponTypeData" variant="outlined"
                        :rules="fieldRule"></v-select>
                    <v-text-field v-model="weaponOrigin" variant="outlined" :rules="fieldRule" label="ប្រភពអាវុធ"
                        required></v-text-field>
                    <v-select v-model="weaponModel" label="ម៉ាកអាវុធ" :items="weaponModelData" variant="outlined"
                        :rules="fieldRule"></v-select>
                    <div class="grid grid-cols-3 gap-5">
                        <v-select v-model="weaponNumber" label="លេខអាវុធ" :items="weaponNumberData" variant="outlined"
                            :rules="fieldRule" class=" col-span-2"></v-select>
                        <v-text-field v-model="bullets" variant="outlined" :rules="fieldRule" label="ចំនួនគ្រាប់"
                            required></v-text-field>
                    </div>
                    <v-select v-model="originCountry" label="ប្រទេសផលិត" :items="originCountryData" variant="outlined"
                        :rules="fieldRule" class=" col-span-2"></v-select>
                </div>
                <div>
                    <div class="py-10">
                        <h3 class="font-semibold text-lg text-[#2C4FBE]">ព័ត៌មានបុគ្គលកាន់កាប់អាវុធ</h3>
                        <div class="w-full h-[1px] bg-[#9D9D9D]" />
                    </div>
                    <div class="grid grid-cols-3 gap-5">
                        <v-text-field variant="outlined" v-model="name" :rules="fieldRule" label="គោត្តនាម និង នាម" required
                            class=" col-span-2"></v-text-field>
                        <v-select variant="outlined" v-model="sex" :items="sexData" :rules="fieldRule" label="ភេទ"
                            required></v-select>
                    </div>
                    <v-text-field variant="outlined" v-model="title" :rules="fieldRule" label="គោរមងារ" required
                        class=" col-span-2"></v-text-field>
                    <div class="grid grid-cols-3 gap-5">
                        <div class="relative col-span-2">
                            <input v-model="birthday" type="date"
                                class="border-solid border-[#ababab] border-[1px] rounded-md h-14 w-full px-3" />
                            <div class="absolute -top-2 left-3 bg-white">
                                <p class="px-2 text-[#8a8a8a]">ថ្ងៃខែឆ្នាំកំណើត</p>
                            </div>
                        </div>
                        <v-select v-model="nationality" label="សញ្ជាតិ" :items="nationalityData" variant="outlined"
                            :rules="fieldRule"></v-select>
                    </div>
                </div>
                <div>
                    <div class="py-10">
                        <h3 class="font-semibold text-lg text-[#2C4FBE]">ឋានៈតួនាទី និងអង្គភាព</h3>
                        <div class="w-full h-[1px] bg-[#9D9D9D]" />
                    </div>
                    <v-text-field variant="outlined" v-model="position" :rules="fieldRule" label="ឋាន:មុខតំណែង"
                        required></v-text-field>
                    <v-text-field variant="outlined" v-model="role" :rules="fieldRule" label="ឋានន្តរសក្តិ"
                        required></v-text-field>
                ...
                <div>
                    <div class="py-10">
                        <h3 class="font-semibold text-lg text-[#2C4FBE]">កាលបរិច្ឆេទផុតកំណត់</h3>
                        <div class="w-full h-[1px] bg-[#9D9D9D]" />
                    </div>
                    <div class="grid grid-cols-2 gap-5">
                        <div class="relative">
                            <input v-model="checkDate" type="date"
                                class="border-solid border-[#ababab] border-[1px] rounded-md h-14 w-full px-3" />
                            <div class="absolute -top-2 left-3 bg-white">
                                <p class="px-2 text-[#8a8a8a]">កាលបរិច្ឋេទកត់ត្រា</p>
                            </div>
                        </div>
                        <div class="relative">
                            <input v-model="produceDate" type="date"
                                class="border-solid border-[#ababab] border-[1px] rounded-md h-14 w-full px-3" />
                            <div class="absolute -top-2 left-3 bg-white">
                                <p class="px-2 text-[#8a8a8a]">កាលបរិច្ឋេទចេញប័ណ្ណ</p>
                            </div>
                        </div>
                        <div class="relative">
                            <input v-model="expireDate" type="date"
                                class="border-solid border-[#ababab] border-[1px] rounded-md h-14 w-full px-3" />
                            <div class="absolute -top-2 left-3 bg-white">
                                <p class="px-2 text-[#8a8a8a]">កាលបរិច្ឆេទផុតកំណត់</p>
                            </div>
                        </div>
                        <v-checkbox v-model="forever" label="អិចិន្ត្រៃយ៍" class="text-[#2C4FBE]"></v-checkbox>
                    </div>
                </div>
                <div>
                    <div class="py-10">
                        <h3 class="font-semibold text-lg text-[#2C4FBE]">អ្នកចុះហត្ថលេខា</h3>
                        <div class="w-full h-[1px] bg-[#9D9D9D]" />
                    </div>
                    <v-select v-model="signature" label="អ្នកចុះហត្ថលេខា" :items="provincesData" variant="outlined"
                        :rules="fieldRule"></v-select>
                </div>
            ...
            <div class="col-start-8 col-span-5 pt-16">
                <div class="md:pt-0 sm:pt-10 ">
                    <div
                        class="rounded-md border-[#ABABAB] border-[1px] border-solid h-72 md:w-[400px] w-full flex items-center justify-center">
                        <v-text v-if="!imageUrl" class="text-2xl text-[#7A7A7A]">រូបភាព</v-text>
                        <v-img v-if="imageUrl" :src="imageUrl" contain></v-img>
                    </div>
                    <v-btn color="primary" @click="pickFile" class="w-[400px] my-5">
                        <p class="text-lg ml-2">បង្ហាញគំរូកាត</p>
                    </v-btn>
                </div>
            </div>
                    ...
                    <svg width="64" height="64" viewBox="0 0 84 80" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <g clip-path="url(#clip0_179_7317)">
                            ...
                            <path
                                ...
                                stroke="white" stroke-width="4" stroke-linecap="round" stroke-linejoin="round" />
                            <path d="M70.5219 43.4102L55.0187 58.2502" stroke="white" stroke-width="4"
                                stroke-linecap="round" stroke-linejoin="round" />
                            <path d="M55.0187 43.4102L70.5219 58.2502" stroke="white" stroke-width="4"
                                stroke-linecap="round" stroke-linejoin="round" />
                        </g>
                        <defs>
...
export default {
    data: () => ({
        ...
        // ព័ត៍មានទូទៅ // 
        cardTypeData: ["អាវុធប្រើប្រាស់", "អាវុធប្រើប្រាស់អង្គភាព", "ប័ណ្ណសំគាល់អាវុធខ្លីយកតាមខ្លួន"],
        weaponTypeData: ["អាវុធខ្លី", "អាវុធវែង"],
        weaponModelData: ["CZ RAMI", "CZ YSUA", "CX UWKS"],
        weaponNumberData: ["001", "002", "003", "004", "005", "006", "007", "008", "009", "010"],
        originCountryData: ["CZECH", "USA", "CHINA", "RUSSIA", "GERMANY", "FRANCE", "ITALY", "UK", "JAPAN", "KOREA", "INDIA", "CAMBODIA"],
        sexData: ["ប្រុស", "ស្រី"],
